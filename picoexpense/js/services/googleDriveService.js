...
const TOKEN_KEY = 'picoexpense.gdrive.accessToken';
const TOKEN_EXP_KEY = 'picoexpense.gdrive.tokenExp';
...
export async function pickDriveFolder() {
  ...
  return new Promise((resolve) => {
    ...
    let builder = new google.picker.PickerBuilder()
      ...
      .setTitle('Choose a Drive folder â€” PicoExpense will use/create PicoExpenseBackup inside it')
      .setCallback((data) => {
...
export async function pickDriveBackupFile() {
  ...
  return new Promise((resolve) => {
    ...
    let builder = new google.picker.PickerBuilder()
      ...
      .setOAuthToken(token)
      .setDeveloperKey(creds.apiKey)
      .setTitle('Choose a PicoExpense backup (.exp.zip or .exp.json)')
      .setCallback((data) => {
        if (data.action === google.picker.Action.CANCEL) {
          resolve(null);
          return;
...
export async function uploadBackupFile(blob, fileName, folderId, opts = {}) {
  ...
  if (opts.exportedAt) {
    metadata.appProperties = { picoexpenseExportedAt: String(opts.exportedAt) };
  }
...
export async function uploadOrUpdateBackupFile(blob, fileName, folderId, opts = {}) {
  if (opts.existingFileId) {
    try {
      ...
      // File may have been deleted â€” fall through to create
...
/**
 * Cached token only â€” does not open a sign-in popup.
 * @returns {string|null}
 */
...
function buildMultipartBody(metadata, blob) {
  const boundary = `picoexpense_${Date.now()}`;
