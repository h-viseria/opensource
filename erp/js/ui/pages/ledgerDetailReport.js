/**
 * Ledger detail report — statement plus target (contra) account full paths.
 */
 ...
 */
export async function renderLedgerDetailReport(ctx, outlet) {
  ...
  const report = await reportService.ledgerDetailReport(defaults.book.id, ledgerId, range);
  ...
  outlet.innerHTML = `
    <div class="page-header">
      <div>
        <p class="page-eyebrow"><a href="#/reports">Reports</a> / Ledger detail</p>
        <h1 class="page-header__title">${escapeHtml(report.ledger.name)}</h1>
        <p class="page-header__desc">
          <span class="mono">${escapeHtml(report.ledgerPath || report.ledger.name)}</span>
    ...
    <form class="toolbar filter-bar" id="ledger-detail-filter">
      <div class="field">
        ...
        <table class="data-table">
            ...
            ${
              report.entries.length === 0
                ...
                <tr class="${rowClass}">
                  ...
                  <td class="mono" title="${escapeHtml(e.targetAccount || '')}">${escapeHtml(e.targetAccount || '—')}</td>
          ...
          <tfoot>
            <tr class="report-total">
              <td colspan="4"><strong>Period movements</strong></td>
  ...
  const form = /** @type {HTMLFormElement|null} */ (outlet.querySelector('#ledger-detail-filter'));
  ...
  const applyFilter = () => {
    ...
    location.hash = `#/reports/ledger-detail?${params.toString()}`;
  };
  ...
  wireReportDownloads(outlet, {
    fileBase: `ledger-detail-${String(report.ledger.name || 'account')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, '')
      .slice(0, 40)}`,
